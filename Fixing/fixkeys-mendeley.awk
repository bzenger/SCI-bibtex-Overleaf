# A program to fix the keys in an existing bib file produced by Mendeley
...
# Now revise this Mendeley key to our SCI convention, assuming this is
# standard Mendeley, which is first author last name and year date.
	  if ( length(oldkey) > 4 ){
#     cnum = match (oldkey, /[^[:digit:]]/, keyarr);
	      cnum = match(oldkey, /[0-9]+/);
#	      printf("cnum is %d in oldkey = %s\n", cnum, oldkey);
	      if ( cnum ) {
		  keylen = length(oldkey);
#		  knum = keylen - 3;
#		  yearstring = substr(oldkey,knum,4);
		  yearstring = substr(oldkey,cnum);
		  if ( keylen > 6 ){
		      firstauth = substr(oldkey,1,3);
# Now the case of authors with names less than 3 characters long		  
		  } else {
		      authlen = keylen - 4;
		      firstauth = substr(oldkey,1,authlen);
		  }
		    key = prefix ":" firstauth yearstring;
#		    printf("firstauth is %s and theyear is %s\n", firstauth, yearstring);

# The case of no number in the string		  
	      } else {
		  key = oldkey;
	      }
	      qfounday = 1;
...
# Now if the original key was not useful, find the first author's last name
# from the author list. 	  
      } else if ( ! qfounday && index(tolower(thelines[linenum]), "author") > 0 ) {
...
# Find the year
      } else if ( ! qfounday && index(tolower(thelines[linenum]), "year") > 0 ) {
